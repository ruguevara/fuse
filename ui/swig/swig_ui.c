...
static libspectrum_word
  image[ DISPLAY_SCREEN_HEIGHT ][ DISPLAY_SCREEN_WIDTH ];
...
ui_init( int *argc, char ***argv ) {
  ...
  tb_printf(0, y++, ZX_BR_WHITE, 0, "Hello from console Fuse");
  tb_printf(0, y++, ZX_BR_WHITE, 0, "width=%d height=%d", tb_width(), tb_height());
  tb_present();
...
ui_event( void )
{
  ...
  tb_printf(0, y + 2, ZX_BR_WHITE, ZX_BR_BLACK, "pressed %d\n", pressed);

  if (pressed) {
      fuse_event.type = INPUT_EVENT_KEYRELEASE;
      input_event( &fuse_event );
      pressed = 0;
      tb_printf(0, y + 1, ZX_BR_WHITE, ZX_BR_BLACK, "release\n", ev.ch);
  } else {
  }
  ...
  while (ev.type == TB_EVENT_KEY) {
      if (ev.ch == 'p') {
        if (paused) {
          ...
          paused = 1;
          fuse_emulation_pause();  /* WTF do not work */
        }
        break;
      }
      ...
      if (ev.ch != 0) {
        fuse_keysym = keysyms_remap( ev.ch );
        tb_printf(0, y + 0, ZX_BR_WHITE, ZX_BR_BLACK, "key: %d -> %d\n", ev.ch, fuse_keysym);
      } else {
        fuse_keysym = keysyms_remap( ev.key );
        tb_printf(0, y + 0, ZX_BR_WHITE, ZX_BR_BLACK, "key: %d -> %d\n", ev.key, fuse_keysym);
      }
  ...
  //   widget_do_media();
  // } else if (c != 0){
  //   // tb_printf(0, 0, TB_GREEN, 0, "hello from termbox %d", c);
  // }

  fuse_emulation_unpause();
  tb_printf(0, y + 3, ZX_BR_WHITE, ZX_BR_BLACK, "paused %d\n", paused);
  tb_printf(20, y + 3, ZX_BR_WHITE, ZX_BR_BLACK, "pausing   %d\n", fuse_emulation_paused);
  /* No error */
  return 0;
}
